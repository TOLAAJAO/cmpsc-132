        ...
        while current:
            if current.value.cid == cid:
                ...
                if current.next:
                    ...
                    self.tail = current.previous
                new_space = self.remainingSpace + old_size - content.size
                if new_space < 0:
                    current.previous = None
                    current.next = self.head
                    if self.head:
...
class Cache:
    ...
    def insert(self, content, evictionPolicy):
        idx = hash(content)
        return self.hierarchy[idx].put(content, evictionPolicy)


    def __getitem__(self, content):
        idx = hash(content)
        lst = self.hierarchy[idx]
        current = lst.head
        while current:
            if current.value.cid == content.cid:
                if current is not lst.head:
                    if current.previous:
                        current.previous.next = current.next
                    if current.next:
                        current.next.previous = current.previous
                    if current is lst.tail:
